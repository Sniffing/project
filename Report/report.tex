...
$newValue \longleftarrow 0$\;
	\For{pixel($i,j$) under $gMask(h,w$)}{
		newValue $\longleftarrow$ newValue + getPixelValue($Img,i,j)$ * $gMask(h,w)$\; 		
	}
...
\begin{landscape}
\begin{table}[h]
\begin{tabular}{p{.2\textwidth}|p{.2\textwidth}|p{.4\textwidth}|p{.25\textwidth}|p{.25\textwidth}}
\hline \hline
...
%getting feedback from a handful
...
below. Note that this distribution does not come with the libraries
...
be downloaded at http://opencv.org/downloads.html. \\
\\
...
\\
The ArUco library can be obtained through http://www.uco.es/investiga/grupos/ava/node/26.
It is worth noting that changes may happen to the library which changes the
API and the function calls. In the even of this and the issue is not resolved,
please leave a message on the GitHub page for an update.\\
\\
Once you have these libraries in place, you can pull from my GitHub 
...
\begin{verbatim}
	...
	make
\end{verbatim}
This should have compiled the project read to be used.
